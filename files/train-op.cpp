...
void model::train(string path)
{
    ...
    {
        model& lm = local_models[tid];

        for (int i = 0; i < lm.preterminals.size(); ++i)
        {
            PT& l_pt = lm.preterminals[i];
            int local_freq = lm.preterm_freq[i];

            int global_id = FindPT(l_pt);
            if (global_id == -1)
            {
                int new_id = GetNextPretermID();
                preterminals.emplace_back(l_pt);
        ...
        for (int i = 0; i < lm.letters.size(); ++i)
        {
            segment& l_seg = lm.letters[i];
            int global_id = FindLetter(l_seg);
            if (global_id == -1)
            {
                int new_id = GetNextLettersID();
                letters.emplace_back(l_seg);
                letters_freq[new_id] = lm.letters_freq[i];
            }
            else
            {
                letters_freq[global_id] += lm.letters_freq[i];
                for (auto& pair : l_seg.values)
                {
                    string val = pair.first;
                    int local_val_id = pair.second;
                    int local_val_freq = l_seg.freqs[local_val_id];

                    letters[global_id].insert(val);
                    int global_val_id = letters[global_id].values[val];
                    // 修正 insert 默认频数累加带来的偏差，补充全部频数
                    letters[global_id].freqs[global_val_id] += (local_val_freq - 1);
                }
            }
        }

        for (int i = 0; i < lm.digits.size(); ++i)
        {
            segment& l_seg = lm.digits[i];
            int global_id = FindDigit(l_seg);
            if (global_id == -1)
            {
                int new_id = GetNextDigitsID();
                digits.emplace_back(l_seg);
                digits_freq[new_id] = lm.digits_freq[i];
            }
            else
            {
                digits_freq[global_id] += lm.digits_freq[i];
                for (auto& pair : l_seg.values)
                {
                    string val = pair.first;
                    int local_val_id = pair.second;
                    ...
                    digits[global_id].insert(val);
                    int global_val_id = digits[global_id].values[val];
                    digits[global_id].freqs[global_val_id] += (local_val_freq - 1);
                }
            }
        }

        for (int i = 0; i < lm.symbols.size(); ++i)
        {
            segment& l_seg = lm.symbols[i];
            int global_id = FindSymbol(l_seg);
            if (global_id == -1)
            {
                int new_id = GetNextSymbolsID();
                symbols.emplace_back(l_seg);
                symbols_freq[new_id] = lm.symbols_freq[i];
            }
            else
            {
                symbols_freq[global_id] += lm.symbols_freq[i];
                for (auto& pair : l_seg.values)
                {
                    string val = pair.first;
                    int local_val_id = pair.second;
                    ...
                    symbols[global_id].insert(val);
                    int global_val_id = symbols[global_id].values[val];
                    symbols[global_id].freqs[global_val_id] += (local_val_freq - 1);
                }
            }
        }
    }
